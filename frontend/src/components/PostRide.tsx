...
const PostRide: React.FC<PostRideProps> = ({ user, onAuthRequired }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Create a mock ride object for demonstration
    const newRide: Ride = {
      id: `ride_${Date.now()}`,
      driver: user,
      from: {
      ...
      currentPricePerSeat: parseInt(formData.price),
      ...
      status: 'active',
      passengers: [],
      shareableLink: `https://ridealong.com/ride/ride_${Date.now()}`,
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-8">
          ...
          <div className="pt-6 border-t border-gray-100">
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-blue-600 to-blue-800 text-white py-4 px-8 rounded-xl font-semibold text-lg hover:from-blue-700 hover:to-blue-900 transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl"
            >
              Publish Your Ride
            </button>
          </div>
